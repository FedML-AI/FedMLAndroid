...
import ai.fedml.iot.uitls.ServiceUtils;
...
abstract class BaseServiceManager {
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            Log.d(TAG, "onServiceConnected, isDestroy = " + isDestroy +
                    ", mServiceConnected = " + mServiceConnected + ", action = " + mServiceAction);
    ...
    private void startAndBindService() {
        Context context = mContext;
        if (context == null) {
            Log.v(TAG, "startAndBindService, context = null, return");
            return;
        }
        String hostPkg = DeviceUtils.getAppPackageName();
        ...
        try {
            if (intent != null) {
                ...
                Log.d(TAG, "bindService startService and bind " + ret);
            } else {
                Log.d(TAG, "can not find intent for action " + mServiceAction + " in " + hostPkg);
            }
        } catch (Exception e) {
            e.printStackTrace();
